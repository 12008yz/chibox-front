  ...
  try {
    ...
    if (newGameState.status === 'finished') {
      if (newGameState.winner === 'player') {
        ...
        // Если приз был выдан, обнуляем попытки на сегодня
        if (rewardGiven) {
          logger.info(`Приз выдан, обнуляем попытки для пользователя ${userId}`);
    ...
    if (result === 'win') {
      if (rewardGiven) {
        message = 'Поздравляем! Вы выиграли и получили бонусный кейс! Приходите завтра за новыми попытками.';
      } else {
        message = 'Вы выиграли, но приз можно получить только один раз в день.';
      }
    } else if (result === 'lose') {

...
const SnakeGameBackground: React.FC<SnakeGameBackgroundProps> = ({ className = '' }) => {
  ...
  // Функция для поиска пути к яблоку
  const findDirectionToFood = useCallback(() => {
    const state = gameStateRef.current;
    ...
    // Находим лучшее направление к еде
    let bestDirection = state.direction;
    let bestDistance = Infinity;

    for (const direction of directions) {
      // Избегаем движения в обратную сторону
      if (direction.x === -state.direction.x && direction.y === -state.direction.y) {
        continue;
      }
      ...
      // Обрабатываем границы (проходим сквозь стены)
      if (newX < 0) newX = state.tileCount.x - 1;
      if (newX >= state.tileCount.x) newX = 0;
      if (newY < 0) newY = state.tileCount.y - 1;
      if (newY >= state.tileCount.y) newY = 0;

      // Проверяем, не столкнемся ли с собой
      const wouldCollide = state.snake.some(segment => segment.x === newX && segment.y === newY);
      if (wouldCollide) {
        continue; // Пропускаем это направление
      }

      // Вычисляем расстояние до еды (Manhattan distance)
  ...
  const updateGame = useCallback(() => {
    ...
    // Находим лучшее направление к еде
    state.direction = findDirectionToFood();
  ...
  }, [resetGame, generateFood, findDirectionToFood]);

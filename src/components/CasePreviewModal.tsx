...
import { CaseItem } from './CasePreviewModal/components/CaseItem';
import { ModalHeader } from './CasePreviewModal/components/ModalHeader';
import { ModalFooter } from './CasePreviewModal/components/ModalFooter';
import { CasePreviewModalProps } from './CasePreviewModal/types';
import { getRarityColor, generateGoldenSparks, getDefaultCaseImage } from './CasePreviewModal/utils';
import { injectStyles } from './CasePreviewModal/styles';
...
const CasePreviewModal: React.FC<CasePreviewModalProps> = ({
  ...
  const [, setShouldFakeSlowdown] = useState(false);
  const [, setShouldStopBetween] = useState(false);
  ...
  const startAnimation = useCallback((wonItem: any) => {
    ...
    const animateSlider = () => {
      ...
      if (useStopBetween && currentAvailablePosition === wonItemIndex - 1) {
        const currentItemInFullList = itemsWithAdjustedChances.findIndex((_, idx) => {

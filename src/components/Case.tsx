...
const Case: React.FC<CaseProps> = ({ title, image, price, fixedPrices = false, description, nextCaseAvailableTime, isBonusCase = false, isRegistrationBonusCase = false, onPlayBonusGame, isTicTacToeCase = false, isAuthenticated = false, onAuthRequired }) => {
  ...
  return (
    ...
    >
      ...
      <div className="relative w-full flex items-end justify-center overflow-visible h-[140px] sm:h-[156px] md:h-auto md:aspect-[3/4]">
        {caseImageUrl ? (
          <img loading="lazy" decoding="async" src={caseImageUrl}
            ...
            height="256"
            draggable="false"
            className={`case-image w-[112%] max-w-none h-full md:w-full md:h-64 object-contain md:object-cover md:-ml-4 relative z-10 transition-all duration-300 select-none ${loaded ? 'opacity-100' : 'opacity-0'}`}
            style={{
              pointerEvents: 'none',
              userSelect: 'none'

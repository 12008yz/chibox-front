...
export function getOptimalImageSize(): string {
  ...
  // Для мобильных устройств (до 640px) используем меньший размер
  if (screenWidth < 640) {
    return '360fx360f';
  }

  // Для планшетов (640px - 1024px) используем средний размер
  if (screenWidth < 1024) {
    return '512fx512f';
  }

  // Для больших десктопов используем максимальный размер
  if (screenWidth >= 1280) {
    return '730fx730f';
  }

  // Для обычного десктопа используем полный размер
  return '512fx512f';
}
